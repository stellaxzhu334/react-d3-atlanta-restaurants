...
import RatingFilter1 from './components/rating_filter1.js';
...
class App4 extends Component {
    ...
    loadRestaurantCSVData() {
        d3.csv(process.env.PUBLIC_URL + '/atlanta_biz_data_change.csv').then((data) => {
    ...
    loadRatingPercentageCSVData() {
        d3.csv(process.env.PUBLIC_URL + '/atlanta_biz_rating_percent_data_change.csv').then((data) => {
            const formattedData = data.map((row) => ({
                restaurant_name: String(row.restaurant_name),
    ...
    render() {
        return (
            <div className="App">
                ...
                <RatingFilter1 onRatingChange={this.handleRatingChange} />
